...
import { UserIcon, CreditCardIcon, BellIcon, LockIcon } from './common/Icons';
...
const Settings: React.FC<{ demoPlan: DemoPlan; setDemoPlan: (plan: DemoPlan) => void; }> = ({ demoPlan, setDemoPlan }) => {
                ...
                {/* Plan Selection Card */}
                 <div className="space-y-4">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-md p-6">
                        <h3 className="font-bold text-lg mb-4">Ganti Paket Langganan</h3>
                        <div className="space-y-3">
                            {(Object.keys(planDetails) as DemoPlan[]).map(plan => (
                                <button key={plan} onClick={() => plan !== demoPlan ? handleUpgradeClick(plan) : undefined}
                                    ...
                                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{planDetails[plan].price}</p>
